...
**Custom/On-Prem Instances:**
```bash copy
export RUNREVEAL_APPBASEURL="https://app.YOUR_BASE_DOMAIN.runreveal.com"
```
...
```bash
Create a new detection using command-line flags
...
Flags:
      --categories strings
      --description string
      --detectiontype string        sql or sigma detection (required)
      --disabled
      --displayname string
      --fields strings
  -h, --help                        help for create
      --mitreattacks strings
      --mitretechniques strings
      --name string                 The detection name (required)
      --notes string
      --notificationnames strings
      --parameters string           JSON format
      --query string                The query to run (required)
      --riskscore int
      --schedule string
...
Usage:
  runreveal detections create-wizard [flags]
...
```bash
Provided a streaming detection, run against provided log events in ndjson format and get whether there would be a match per line.
