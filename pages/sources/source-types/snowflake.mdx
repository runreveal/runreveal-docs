...
If your Snowflake account has IP allowlisting enabled, you'll need to whitelist RunReveal's outbound IP addresses before the integration can connect. See [Network Connectivity and IP Whitelisting](/reference/network-connectivity) for the IP addresses to add to your Snowflake network policy allowlist.
