...
| **Cloud Provider** | Platform identification | `aws`, `azure`, `gcp` |
| **Functional** | Action type | `password-change`, `privilege-escalation` |
...
## SQL vs Sigma Detections

RunReveal supports two types of detections: **SQL** and **Sigma**. Understanding the differences helps you choose the right approach for your use case.

<Tabs items={['SQL Detections', 'Sigma Detections']}>

<Tabs.Tab title="SQL Detections">

SQL detections are scheduled queries that execute against your log data at regular intervals. They provide full SQL query capabilities and can perform complex analysis, aggregations, and joins.
...
<Tabs.Tab title="Sigma Detections">

Sigma detections are streaming rules that evaluate events as they are ingested into RunReveal. They use the standardized Sigma rule format and provide real-time detection capabilities.
...
| Feature | SQL Detections | Sigma Detections |
|---------|----------------|------------------|
