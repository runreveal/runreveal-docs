...
All query detections should include one of the following:

**Option 1:** A `HAVING` clause with timestamps:
```sql
HAVING (receivedAt >= {from:DateTime}) AND (receivedAt <= {to:DateTime})
```

**Option 2:** A `WHERE` clause with timestamps:
```sql
...
**Noie**: If you are using legacy fields prefixed with `normalized`, there is backwards compatibility to still find those fields.
